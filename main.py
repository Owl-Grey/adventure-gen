...
gang_prom=["наркоторговля","работорговля","рэкет","воровство","убийства","анархизм"]
...
class Pop(Popup):
    ...
    def set_description(self, title):
        if title == "Броня":
            ...
            strx=""
            k=0
            for j in equip[0][0][0]:
                k=k+1
                strx+=str(k)+". "+j+"\n"
            self.list.text=str(strx)
        elif title == "Одежда Верх":
            strx=""
            k=0
            for j in equip[0][0][1]:
                k=k+1
                strx+=str(k)+". "+j+"\n"
            self.list.text=str(strx)
        elif title == "Одежда Низ":
            strx=""
            k=0
            for j in equip[0][0][2]:
                k=k+1
                strx+=str(k)+". "+j+"\n"
            self.list.text=str(strx)
        elif title == "Обувь":
...
class Container(TabbedPanel):
    ...
    def get_chmot(self, *args):
        ...
        self.ids.chmot.add_widget(ToggleButton(
            text="Верх",
            group="Одежда Верх",
            on_press=self.clk,
            size_hint_y=None,
    ...
    def gen(self, *args):
        ...
        self.rand_wear()
    def gungs(self):
        prl=["Прекрасные","Ухоженные","Забавные","Несчастные","Боевые","Странные","Конченные","Антисоциальные","Социальные","Герметичные","Проникновенные","Стальные","Кривые","Несломленные","Сломленные","Порванные","Подземные","Радикальные","Двоичные","Хрустальные","Яростные","Электро","Плазмо","Мобильные","Био","Шаолиньские","Хромированные","Полимерные","Деловые","Смертельные","Жизненные","Теневые","Сверхзвуковые","Химические","Избранные","Токсичные","Стволы и","Лишние","Железные","Вмазанные","Уличные","Суши и","Социалистические","Коммунистические","Плавные","Шустрые","Сатанистские","Готические","Садистские"]
        sus=["Акулы","Парни","Маньяки","Короли","Легенды","Убийцы","Пули","Револьверы","Боссы","Головорезы","Пауки","Расисты","Алкоголики","Наркоманы","Воры","Паркуристы","Хромеры","Мотоциклисты","Киборги","Рэкетиры","Бездомные","Нигилисты","Недосатанисты","Эмоготы","Канибаллы","Драконы","Неудачники","Социалы","Кривые","Прямые","Ботаники","Драчуны","Дрочеры","Задроты","Гики","Болтуны","Самураи","Гопники","Инкасаторы","Пертурбаторы","Шарфы","Змеи","Псы","Ассасины","Пророки","Горгульи","Машины","Драконы","Фрики","Девки","Звери","Жандармы","Полицейские","Потрошители","Дикари","Бунтари","Дети апокалипсиса","Штормы","Солдаты","Культисты","Детки","Призраки","Священники","Клоуны","Кулаки","Ножи"]
        name=""
        name="Банда " + choice(prl)+" "+choice(sus)+ ". Деятельность: " +choice(gang_prom) +". Местонахождение: " + choice(points)
        gungs.append(name)
        return name
    ...
    def randct(self,l):
        ...
        prom=["оружие","одежда","аугментации","автомобилестроение","энергетика","топливо"]
        listl=[]
        strl=""
        dstr=""
        k=0
        if l == 1:
            ...
            if specs =="Промышленность":
                ...
                rez=". \nОсновной деятельностью является " + str(choice(gang_prom))
            ...
            strl+="\nСпециализация города: " + str(specs) + str(rez) +"." + gn + "\nВсего в городе " + str(len(set(listl))) + " районов:\n"
            ir=0
            for i in set(listl):
                ir+=1
                mn = randint(1,3)
            ...
            if gnch>0:
                ...
                for i in range(gnch):
                    gn+=self.gungs()+"\n"
                    if i == gnch-1:
                        gn+="И, в том числе множество скрытых и тайных.\n"
            kr = randint(5,7)
            for i in range(kr):
                ret=choice(distr)
                listl.append(ret)
